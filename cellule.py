...
import pygame
from pylab import sqrt,dist,vdot,dot,floor,array,randn,randint,sqrt,log,ceil
...
LARGEUR = 500
LONGUEUR = 800
DELTA_T = 1.5	# intervalle de temps entre deux rafraîchissement de la position : 0.0505 0.5
...
RAYON = 15	# rayon des boules, en pixel
...
def derivee_lennard_jones_pot(r):#derivee du potentiel de Lennard-Jones
	return (r-2*RAYON)
	# derlennardJones = (-384*(RAYON**6)/r**13)+(1/r**7) #*f_indicatrice_rayonA(r)
...
class Boule:
	...
	def affiche(self,ecran):
		...
		# print(self.image)
		# print((self.x,self.y))
		# ecran.blit(self.image,pygame.Rect(self.x+RAYON,self.y-RAYON,2*RAYON,2*RAYON))
	...
	def trouverVoisins(self,g):#g est la grille divisant l'espace 
		...
		for i in range(1,6):#parcourir les boites adjacentes a la boite k
			if(i != 2):
				...
				if(1 <= a <= N_X*N_Y):
					# print("k_Xi -",i,"=",a)
					L_cell_peut_etre_voisines += g.grille[k_Xi-i-1] #concatener des listes, rajouter les boules de la boite  a la liste des cellules peut etre voisines
				b = k_Xi+i
				if(1 <= b <= N_X*N_Y):
					L_cell_peut_etre_voisines += g.grille[k_Xi+i-1] #-1 pour rester dans [0,15], decalage de 1 entre vrai indice et indice python
	...
	def nouvelle_position(self,L_voisins):
		...
		nb_cell_voisines = len(L_voisins)#nombre de celles voisines

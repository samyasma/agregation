...
import pygame
from trouver_cases_adjacentes import *
...
LARGEUR = 500
LONGUEUR = 800
DELTA_T = 0.05	# 0.05 -pot_normal : intervalle de temps entre deux rafraîchissement de la position
...
RAYON_A = 4*RAYON #4*RAYON rayon d'attraction des cellules
...
def derivee_potentiel(r):#derivee du potentiel de Lennard-Jones
	return((r-2*RAYON))	
	# derlennardJones = (-384*(RAYON**6)/r**13)+(6/r**7) #*f_indicatrice_rayonA(r)
...
class Grille:
	...
	def __init__(self,L_boules):
		...
		#on cree une grille vide de dimensions N_X*N_Y, de N_X*N_Y boites
		for k in range(N_X):
			for j in range(N_Y):
				self.grille.append([])
		# print("grille vide")
		...
		for elt in L_boules:
			# print(str(elt))
			Xi_x,Xi_y = float(elt.x),float(elt.y)
			ix = int(floor((Xi_x-X_MIN)/DELTA_X))+1
			iy = int(floor((Xi_y-Y_MIN)/DELTA_Y))+1

			# ix = int(floor(float((Xi_x-X_MIN)/DELTA_X)))+1
			# iy = int(floor(float((Xi_y-Y_MIN)/DELTA_Y)))+1

			# print(ix,iy)
			k = ix + (iy-1)*N_X  #on calcule k, le numero de la boite qui la contient
			# print("k =",k)
			if(1 <= k <= N_X*N_Y):
				self.grille[k-1].append(elt) #on ajoute la cellule ds la bonne boite, k-1 car on commence a l'indice 0 (pas 1)
	...
	def __str__(self):
		...
		for boite in self.grille:
			s += "Boite "+str(i)
			s += "["
			nb_cellules_dans_la_boite = 0
			for cell in boite:
				s += str(cell)+","
				nb_cellules_dans_la_boite += 1
				nb_total_cellules += 1
			s += "] "+str(nb_cellules_dans_la_boite)+" cellules \n"
			i+=1
		s += "] grille avec"+" "+str(len(self.grille))+" cases et "+str(nb_total_cellules)+" cellules"
		return(s)
...
class Boule:
	...
	def __init__(self,nom, position, nomFichierImage):
		...
		self.image = pygame.transform.scale(surface,(RAYON,RAYON))
	...
	def trouverVoisins(self,g):#g est la grille divisant l'espace 
		...
		#exemple : on veut les adjacents de k_Xi = 6, il faut ecrire L_adjacentsunD[7] = L_adjacentsunD[k_Xi-1]
		#L_adjacentsunD[k] donne la liste des adjacents de k+1
		# print("L_adjacents_unD[k_Xi] =",L_adjacents_unD[k_Xi-1])
		# print("k_Xi =",k_Xi)
		k_Xi
		if(1<=k_Xi<=16):		
			for k in L_adjacents_unD[k_Xi-1]: 
				L_cell_peut_etre_voisines += g.grille[k-1]
			# print("k adj =",k)		
		...
		# 		if(1 <= a <= N_X*N_Y-1):
		# 			# print("k_Xi -",i,"=",a)
		# 			L_cell_peut_etre_voisines += g.grille[k_Xi-i] #concatener des listes, rajouter les boules de la boite  a la liste des cellules peut etre voisines
		# 			print(a)
		# 		b = k_Xi+i
		# 		if(1 <= b <= N_X*N_Y-1):
		# 			L_cell_peut_etre_voisines += g.grille[k_Xi+i] #-1 pour rester dans [0,15], decalage de 1 entre vrai indice et indice python
		# 			print(b)
	...
	def nouvelle_position(self,L_voisins):
		...
		for k in range(nb_cell_voisines):
			...
			if(distance_self_voisin != 0):#exclut le cas ou Xi = Xj
				nextPosition = nextPosition + (derivee_potentiel(distance_self_voisin)/distance_self_voisin)*array([float(self.x-voisin.x),float(self.y-voisin.y)])
	...
	def distance(self,b):
		# print(b.x,self.x,b.y,self.y)
		distance = sqrt((b.x-self.x)**2+(b.y-self.y)**2)
		return(distance)			

#####ON N'EST PLUS DANS LA CLASSE BOULE######
...
def nouvelles_positions(L_positions,g):
	...
	for cell in L_positions:#on parcourt toutes les boules pour calculer leur nouvelle position
		# print("cell =",str(cell))
...
def nouvelles_positions_sans_grille(L_positions,g):
	...
	for cell in L_positions:#on parcourt toutes les boules
		# print("cell =",str(cell))
		L_nouvelles_positions.append(cell.nouvelle_position_sans_grille(L_positions))#a partir de sa liste de voisins, on calcule la nouvelle position
	for k in range(len(L_positions)):#on actualise la liste de position avec les nouvelles positions
		L_positions[k].x,L_positions[k].y = L_nouvelles_positions[k]
	# afficher_liste_boules(L_nouvelles_positions)
	return(L_positions)
...
def afficher_liste_boules(L_boules):
	...
	for cell in L_boules:
		print(str(cell)+",",end='')
	print("]",end='')
	s = " liste avec "+str(len(L_boules))+" cellules\n"
	print(s,end='')


######TEST#######
# X1 = Boule("X1",(172,67),"boule_blanche.png") 
# X2 = Boule("X2",(207,67),"boule_blanche.png") 
# X3 = Boule("X3",(242,67),"boule_blanche.png")  
# L_pos = [X1,X2,X3]
# g = Grille(L_pos)
# # 
